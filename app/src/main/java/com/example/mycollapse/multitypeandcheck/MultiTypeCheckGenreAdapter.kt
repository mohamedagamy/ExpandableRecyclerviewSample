...
import com.example.mycollapse.expand.ArtistViewHolder
import com.example.mycollapse.expand.GenreViewHolder
import com.example.mycollapse.singlecheck.SingleCheckArtistViewHolder
...
class MultiTypeCheckGenreAdapter(groups: List<ExpandableGroup<*>?>?) :
    MultiTypeExpandableRecyclerViewAdapter<GenreViewHolder, ChildViewHolder>(groups),
    ...
    override fun onCreateChildViewHolder(parent: ViewGroup, viewType: Int): ChildViewHolder {
        return when (viewType) {
            ARTIST_VIEW_TYPE -> {
                val artist = LayoutInflater.from(parent.context)
    ...
    override fun onBindChildViewHolder(
        holder: ChildViewHolder, flatPosition: Int, group: ExpandableGroup<*>,
        childIndex: Int
    ) {
        ...
        when (viewType) {
            ...
            FAVORITE_VIEW_TYPE -> {
                ...
                (holder as SingleCheckArtistViewHolder)
                    .onBindViewHolder(
                        flatPosition,
                        childCheckController.isChildChecked(listPosition)
                    )
    ...
    override fun onChildCheckChanged(view: View, checked: Boolean, flatPos: Int) {
        ...
        childClickListener?.onCheckChildCLick(
            view, checked,
        ...
        for (i in groups.indices) {
            ...
            if (isGroupExpanded(group)) {
                notifyItemRangeChanged(
                    expandableList.getFlattenedFirstChildIndex(i),
                    group.itemCount
                )
            }
        }
